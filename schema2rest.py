...
def main():
    ...
    # convert the schema to a format that can be used by the generators (xxx.yaml)
    convert_schema(mmd_file, path_root)

    # use the yaml to create the code and update indexes for uniques 
    yaml_file = mmd_file.replace(".mmd", ".yaml")
    generate_code(yaml_file, path_root)
    update_indexes(yaml_file)
